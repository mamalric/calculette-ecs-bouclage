...
## État actuel
Version v24 en ligne sur GitHub Pages, étiquetée sur `main` (le développement continue sur la branche `dev`). 7 usages (logement, bureaux, hôtel, restaurant, santé, scolaire, sport), stockage par méthode COSTIC (logement) ou bilan de pointe avec choix du mode de production et comparatif des quatre modes (tertiaire), maintien en température en trois modes (réseau bouclé, traçage électrique, sans maintien) avec architectures COSTIC et découpage en boucles, multi-scénarios, vérification d'existant, impression, thème clair/sombre et panneau "À propos" (rouage) repris du Sélectionneur de radiateurs, aides contextuelles derrière un bouton (i), schémas de principe du réseau, enregistrement et ouverture du projet en JSON. Toutes les valeurs sont sourcées (docs/methodes.md) ; les hypothèses non normées sont signalées dans l'interface. Vérifiée sur les 7 usages et les 4 modes dans le navigateur.

## Prochaine étape
Confronter les résultats à un projet BET réel et ajuster les hypothèses signalées (fractions de pointe et fenêtres de recharge en tertiaire, ratio hôtel 1-2 étoiles) ; pistes suivantes : pertes annuelles de bouclage en kWh, tracés du schéma distinguables sans la couleur.

## Utilisation
En ligne : https://mamalric.github.io/calculette-ecs-bouclage/ (GitHub Pages, publié depuis `main`). Hors ligne : ouvrir index.html dans un navigateur, le fichier est autonome et ne fait aucune requête externe. Les scénarios sont conservés dans le navigateur (localStorage), séparément pour chaque origine : ceux saisis sur le site en ligne et ceux saisis sur le fichier local ne se mélangent pas. Le bouton "Enregistrer" en produit un fichier JSON, que "Ouvrir" relit : c'est ce qui permet de ranger un projet avec les pièces de l'affaire, de le reprendre sur un autre poste ou de le passer à un collègue.
