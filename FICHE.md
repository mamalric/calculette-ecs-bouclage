...
## État actuel
Projet initialisé, dépôt GitHub créé (compte mamalric). Cadrage en cours : types de bâtiments v1 et méthodes de calcul à valider avec l'utilisateur avant tout développement.

## Prochaine étape
Valider le cadrage (types de bâtiments prioritaires, méthodes de calcul par usage, hypothèses par défaut), puis modéliser les données de calcul.

## Utilisation
Aucun prérequis : ouvrir index.html dans un navigateur (fichier autonome, fonctionne hors ligne). Le fichier n'existe pas encore à ce stade.
