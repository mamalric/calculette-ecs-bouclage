...
## État actuel
Version v4 en ligne, étiquetée sur `main` (le développement continue sur la branche `dev`). 7 usages (logement, bureaux, hôtel, restaurant, santé, scolaire, sport), stockage par méthode COSTIC (logement) ou bilan de pointe avec choix du mode de production et comparatif des quatre modes (tertiaire), maintien en température en trois modes (réseau bouclé, traçage électrique, sans maintien) avec architectures COSTIC et découpage en boucles, multi-scénarios, vérification d'existant, impression, thème clair/sombre et panneau "À propos" (rouage) repris du Sélectionneur de radiateurs, aides contextuelles derrière un bouton (i). Toutes les valeurs sont sourcées (docs/methodes.md) ; les hypothèses non normées sont signalées dans l'interface. Vérifiée sur les 7 usages et les 4 modes dans le navigateur.

## Prochaine étape
Confronter les résultats à un projet BET réel et ajuster les hypothèses signalées (fractions de pointe et fenêtres de recharge en tertiaire, ratio hôtel 1-2 étoiles) ; pistes suivantes : pertes annuelles de bouclage en kWh, export JSON des scénarios, table de diamètres propre au PVC-C et au multicouche.
